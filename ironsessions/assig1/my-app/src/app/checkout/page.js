...
export default function CheckoutPage() {
  ...
  // Fetch user session and cart items on page load
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (data.session && data.session.email) {
          setCustomerEmail(data.session.email); // Set email from session
        } else {
          setErrorMessage("No email found. Please log in.");
        }
      } catch (error) {
        console.error("Error fetching user session data:", error);
        setErrorMessage("Error fetching user session data.");
      }
    };
    ...
    const fetchCartItems = async () => {
      try {
        if (customerEmail) {
          const res = await fetch(`/api/cart?email=${customerEmail}`);
    ...
    fetchUserData().then(fetchCartItems);
  }, [customerEmail]);
